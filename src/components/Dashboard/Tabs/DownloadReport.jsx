...
//                     <div>
//                         <h1 className="text-2xl font-black text-slate-900  leading-none">Expenses & Advances Reports</h1>
//                         <p className="text-slate-500 text-xs font-bold uppercase mt-1 tracking-wider">{currentEmployeeName}</p>
//                     </div>

//                     {history.length > 0 && (
//                         <div className="flex gap-2 w-full sm:w-auto">
//                             <button onClick={exportExcel} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white border border-slate-200 px-4 py-3 rounded-xl font-black text-[10px] hover:bg-slate-100 transition-all shadow-sm uppercase tracking-widest">
//                                 <FileSpreadsheet size={16} className="text-emerald-600" />Download Excel
//                             </button>
//                             <button onClick={exportPDF} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white border border-slate-200 px-4 py-3 rounded-xl font-black text-[10px] hover:bg-slate-100 transition-all shadow-sm uppercase tracking-widest">
...
//                                         </div>
//                                         <div>
//                                             <p className="text-lg font-black text-slate-900 er leading-none mb-1">₹{item.amount.toLocaleString('en-IN')}</p>
//                                             <p className="text-slate-600 text-xs font-bold line-clamp-1">{item.category || item.purpose}</p>
//                                             <p className="text-[10px] text-slate-300 font-black uppercase mt-1 er">
...
export default function DownloadReport() {
    ...
    // 🧮 Summary Calculations: Total Expenses, Total Advances, Cross (Net) Amount
    const summary = useMemo(() => {
        ...
        const diff = totalAdvances - totalExpenses; // Advances - Expenses
        const crossAmount = Math.abs(diff); // never show negative

        return {
            totalExpenses,
            totalAdvances,
            crossAmount,
            // When diff >= 0, advances exceed expenses → recoverable from employee
            // When diff < 0, expenses exceed advances → payable to employee
            crossLabel: diff >= 0 ? "Recoverable from Employee" : "Payable to Employee",
    ...
    const exportExcel = () => {
        ...
        data.push({});
        data.push({ "Employee": "Total Expenses", "Amount": summary.totalExpenses });
        data.push({ "Employee": "Total Advances", "Amount": summary.totalAdvances });
        data.push({ "Employee": `Cross Amount (${summary.crossLabel})`, "Amount": summary.crossAmount });
    ...
    const exportPDF = () => {
        ...
        doc.setFont(undefined, 'bold');
        doc.text("Total Expenses (INR):", 14, y);
        doc.text(summary.totalExpenses.toLocaleString('en-IN'), 170, y);
        y += 7;
        doc.text("Total Advances (INR):", 14, y);
        doc.text(summary.totalAdvances.toLocaleString('en-IN'), 170, y);
        y += 7;
        doc.text(`Cross Amount (${summary.crossLabel}):`, 14, y);
        doc.text(summary.crossAmount.toLocaleString('en-IN'), 170, y);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xl font-black text-slate-900">₹{summary.totalExpenses.toLocaleString('en-IN')}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Advances</p>
                                <p className="text-xl font-black text-slate-900">₹{summary.totalAdvances.toLocaleString('en-IN')}</p>
                            </div>
                        </div>

                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex items-center gap-3">
                            <div className={`p-2.5 rounded-xl ${summary.isRecoverable ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                <Scale size={18} />
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cross Amount</p>
                                <p className="text-xl font-black text-slate-900">₹{summary.crossAmount.toLocaleString('en-IN')}</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">{summary.crossLabel}</p>
                            </div>
                        </div>
                    </div>
                )}
